...
4.2 window.location.hash:hash 属性是一个可读可写的字符串，该字符串是 URL 的锚部分（从 # 号开始的部分）。
...
#### 64.（待）了解宏任务和微任务吗，请各举几个常见的/宏任务与微任务哪个先执行？
...
#### 41.前端templating(Mustache, underscore, handlebars)是干嘛的, 怎么用？
...
#### 20.（待）HTTP协议中，GET和POST有什么区别？分别适用什么场景 ？
...
#### 9.（待）**请简述下Vuex的原理和使用方法**

```
https://www.jianshu.com/p/e54a9a34a773
```

#### 10.（待）**请谈谈Vue框架和Angular.js和React的不同**

#### 11.（待）双向绑定原理

#### 12.（待）vue3.0和2.0双向绑定的区别，这样的改动有什么好处？

#### 13.（待）vue-router的实现原理，history和hash模式有什么区别？

#### 14.（待）虚拟dom为什么效率高？

#### 15.（待）请描述vue生命周期

#### 16.（待）keep-alive是否与普通组件有一样的生命周期，如果不是，它有哪些钩子？

#### 17.（待）vue如何实现seo/为什么会被seo搜索到，ssr的生命周期与普通vue生命周期有什么不一样？

#### 18.（待）computed如何得知数据变化/computed有缓存吗？

#### 19.（待）$nextTick原理

#### 20.（待）vue父子组件传参有哪些方式？

#### 21.（待）v-if和v-show有什么区别？